...
class ListAnniversaryPage extends StatefulWidget {
  const ListAnniversaryPage({super.key});

  @override
  State<StatefulWidget> createState() => ListAnniversaryPageState();
}

class ListAnniversaryPageState extends State<ListAnniversaryPage> {
  ...
  onUploaded(String file) async {
    var result = await Services()
        .upload(stores.user.value.id, stores.user.value.idQoo, file);
    if (result['success']) {
            ...
            child: SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  EditBottons(onDeletePress: () {}, onSavePress: onSubmitPress),
                  SizedBox(
                    height: 12,
                  ),
                ],
